...
class GameRoom(BaseEntity, GameObject, iRoomRules):
	...
	def selfConfirmOperation(self, avt_mb, aid, tile_list, isFromClient):
		...
		if len(self.wait_op_info_list) <= 0 or sum([1 for waitOpDict in self.wait_op_info_list if (waitOpDict["idx"] == idx and waitOpDict["state"] == const.OP_STATE_WAIT)]) <= 0:
			avt_mb.doOperationFailed(const.OP_ERROR_NOT_CURRENT)
			return
		if self.players_list[idx].state == const.DISCARD_FORCE and isFromClient:
			...
			for i, p in enumerate(self.players_list):
				if p is not None and len(notifyOpList[i]) > 0 and p.state == const.DISCARD_FORCE:
					...
					if wait_op_dict is not None:
						self.selfConfirmOperation(p.mb, wait_op_dict["aid"], wait_op_dict["tileList"], False)
					else:
						self.selfConfirmOperation(p.mb, const.OP_PASS, notifyOpList[i][0]["tileList"], False)
