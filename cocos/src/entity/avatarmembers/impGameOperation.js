...
var impGameOperation = impCommunicate.extend({
	...
	postOperation : function(serverSitNum, aid, tileList){
		...
		if(aid == const_val.OP_PASS){
			...
			if(this.serverSitNum == serverSitNum){
				...
				} else if (this.curGameRoom.discardStateList[this.serverSitNum] == const_val.DISCARD_FORCE) {
			...
			if(this.serverSitNum == serverSitNum){
				...
				if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
					...
					h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "show_extra_panel", canWinTiles.length > 0 && this.curGameRoom.discardStateList[this.serverSitNum] == const_val.DISCARD_FREE ? true : false);
	...
	setDiscardState : function(state){
		...
		this.baseCall("setDiscardState", state)
	},
	...
	postPlayerDiscardState : function (idx, state){
        ...
        if(idx == this.serverSitNum){
            if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
                h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "force_discard");
            }
        } else {
            if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
                h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "update_player_hand_tiles", idx);
            }
		}
	},
