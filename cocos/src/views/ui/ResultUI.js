	...
	show_by_info:function(finalPlayerInfoList, curGameRoom){
		...
		this.show(function(){
			...
			if (curGameRoom.club_id > 0) {
				clubStr = "    亲友圈号：" + curGameRoom.club_id;
			}
            ...
            if(!((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative) || switches.TEST_OPTION)) {
                ...
                var title = curGameRoom.club_id > 0 ? '亲友圈号【' + curGameRoom.club_id.toString() + '】,招募群主,1000红包奖励群主!' : '房间号【' + curGameRoom.roomID.toString() + '】,招募群主,1000红包奖励群主!';
                var desc = result_str;
