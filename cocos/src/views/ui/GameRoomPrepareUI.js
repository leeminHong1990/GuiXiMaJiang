...
var GameRoomPrepareUI = UIBase.extend({
	...
	initUI:function(){
		...
		var roominfo_list = [["非东带庄,","东带庄,"], ["无摸宝,","摸一宝,","摸二宝,"], ["手动准备", "自动准备"]];
		var share_title = '贵溪麻将 ' + (player.curGameRoom.club_id > 0 ? '亲友圈号【' + player.curGameRoom.club_id.toString() + '】' : '房间号【' + player.curGameRoom.roomID.toString() + '】');
		var share_desc = (roominfo_list[0][player.curGameRoom.game_mode]) + (player.curGameRoom.maxRound + '局,') + (roominfo_list[1][player.curGameRoom.luckyTileNum]) + (player.curGameRoom.max_lose === 9999 ? '无封顶,' : player.curGameRoom.max_lose + '分封顶,') + (roominfo_list[2][player.curGameRoom.hand_prepare]);
