...
var GameRoomUI = UIBase.extend({
	...
	init_extra_panel:function(){
		...
		function extra_btn_1_event(sender, eventType){
			if (eventType === ccui.Widget.TOUCH_ENDED) {
                ...
                // self.touch_state = const_val.PLAYER_TOUCH_FORCE_STATE;
		...
		var player = h1global.player();
		if((player.curGameRoom.handTilesList[player.serverSitNum].length)%3==1){
            let canWinTiles = player.getCanWinTiles();
            this.show_extra_panel(canWinTiles.length > 0 && player.curGameRoom.discardStateList[player.serverSitNum] == const_val.DISCARD_FREE ? true : false);
		}
	...
	lock_player_hand_tiles:function(){
		if(!this.is_show) {return;}
        var player = h1global.player();
        if (player.curGameRoom.discardStateList[player.serverSitNum] === const_val.DISCARD_FORCE) {
            this.touch_state = const_val.PLAYER_TOUCH_FORCE_STATE;
        } else {
	...
	startBeginAnim:function(startTilesList, diceList, dealerIdx){
		...
		function playAnimation(){
			...
			var step3 = cc.CallFunc.create(function(){
				...
				let canWinTiles = player.getCanWinTiles();
				self.show_extra_panel(canWinTiles.length > 0 && player.curGameRoom.discardStateList[player.serverSitNum] == const_val.DISCARD_FREE ? true : false);
                cc.audioEngine.playEffect("res/sound/effect/deal_tile.mp3");
			});
	...
	update_player_hand_tiles: function (serverSitNum, tileList) {
        ...
        if (discardState === const_val.DISCARD_FORCE) {
            options.tilePanelDownOffsetFunc(upTilesList.length , cur_player_tile_panel);
        } else {
        ...
        for (var i = 0; i < 14; i++) {
            ...
            if (discardState === const_val.DISCARD_FORCE) {
            	if(idx == 3){
                    tileDownPositionFunc(tile_img, i, upTilesList.length);
                }else {
                    tileDownPositionFunc(tile_img, i);
                }
            } else {
            ...
            if(handTilesList[i] !== undefined){
                if(handTilesList[i] > 0){
                    ...
                    if(isPlayer){
                        ...
                        if(discardState === const_val.DISCARD_FORCE) {
                        	if(options.mahjongDownImgPathList){
                                tile_img.loadTexture(cc.formatStr(mahjong_down_str, options.mahjongDownImgPathList[(i + upTilesList.length * 3) % 14]), ccui.Widget.PLIST_TEXTURE);
                            }else {
                                tile_img.loadTexture(mahjong_down_str, ccui.Widget.PLIST_TEXTURE);
                            }
                            if(options.tileOrderList){
                                tile_img.setLocalZOrder(options.tileOrderList[(i + upTilesList.length * 3) % 14]);
                            }
                            if(options.tileFlippedX !== undefined){
                                tile_img.setFlippedX(options.tileFlippedX);
                            }
                            if(options.tiledownscale && idx == 3){
                                tile_img.setScale(options.tiledownscale[i + upTilesList.length * 3] * 0.01);
                            }
                            if(options.tilescale && idx == 1){
	...
	update_player_discard_tiles:function(serverSitNum){
        ...
        for(var i = 0; i < deskSize; i++){
            ...
            if(i < maxDeskSize && discardTilesList[i] > 0){
                ...
                tile_img.setVisible(true);
                if(player.curGameRoom.kingTiles == discardTilesList[i]){
    ...
    update_operation_panel:function(op_dict, from_type){
        ...
        for (var i = const_val.SHOW_OP_LIST.length - 1; i >= 0; i--) {
            if (show_op_dict[const_val.SHOW_OP_LIST[i]]) {
                ...
                op_btn.setPosition(cc.p(operation_panel.getContentSize().width - 40 -150 * (operation_btn_list.length-1), operation_panel.getContentSize().height/2))
            }
		}
